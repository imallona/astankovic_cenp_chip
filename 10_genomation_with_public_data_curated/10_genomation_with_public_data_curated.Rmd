---
title: "CENP ChiP-seq and other epigenomic (public) data, sample swap corrected, gene promoters only"
...
head(all_proms$tx_name)
all_proms$symbol <- mapIds(org.Mm.eg.db,
                           keys = sapply(strsplit(all_proms$tx_name, '.', fixed = TRUE),
...
test new mm9 data h3.3 - mm9 bamfiles named `rmdup` only to avoid read dup biases
...
## Stratified metaplots / export gene names by cluster
...
for (samp in names(mat.list)) {
    ...
    for (cl in unique(group.vector)) {
        idx <- group.vector == cl
        tmp <- data.frame(tidyr::pivot_longer(data = as.data.frame(mat.list[[samp]][idx,]),
                                   cols = 1:ncol(mat.list[[samp]]),
                                   names_to = 'pos'),
                          cluster = cl,
                          sample = samp)
        longer[[samp]][[cl]] <- tmp
...
ggplot(data = head(longer, 10000), aes(x = pos, y = value, col = cluster)) +
...
names(edge) <-  gsub('edgeR_dge_results_', '', basename(edge_fns))


if (DOWNSAMPLE){
    for (item in names(edge)) {
        seqlevelsStyle(edge[[item]]) <- 'UCSC'

        edge[[item]] <- keepSeqlevels(edge[[item]], "chr19", pruning.mode="coarse")
    }
} else {
        for (item in names(edge)) {
        seqlevelsStyle(edge[[item]]) <- 'UCSC'

        edge[[item]] <- keepSeqlevels(edge[[item]], allowed_chr, pruning.mode="coarse")
    }
}
...
edge_sm <- lapply(edge, function(x) ScoreMatrixBin(x, genes_proms,
                                                is.noCovNA = TRUE,
                                                bin.num = 50,
                                                type = "auto",  strand.aware = TRUE))
