...
```{r}
covs <- lapply(covs, function(x) ScoreMatrixBin(x, all_proms,
                                              bin.num = 50,
                                              type = "auto",  strand.aware = TRUE))



covs <- ScoreMatrixList(covs, bin.num = 50)
...
```{r}
stopifnot(length(all_proms$tx_name) == length(clu))

for (cl in unique(clu)) {
    idx <- clu == cl
    write.csv(x = data.frame(all_proms[idx,]),
