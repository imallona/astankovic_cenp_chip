...
if (DOWNSAMPLE) {
    ...
    zoomed_proms <- keepSeqlevels((zoomed_proms, allowed_chr, pruning.mode="coarse")
}
...
```{r gse_loop, eval = TRUE, fig.width = 70, fig.height = 10, results = 'asis'}
PUBLIC <- '/home/imallona/cenp_chip/public/'

## gses <- list.files(PUBLIC, include.dirs = TRUE, recursive = FALSE)
gses <- c('GSE52386', 'GSE82015', 'GSE93011')

poster <- list('GSE52386' = c('GSM1264366_Forebrain_P56_H3K27ac_rep1.bed.gz'),
               'GSE82015' = c('GSM2180001_E4_rep4.bed.gz'),
               'GSE93011' = c('GSM2442440_5789_Neuron_PolII_Rep1.bedGraph.gz',
...
for (gse in gses) {
    ...
    fns <- grep('input', fns, invert = TRUE, value = TRUE)
    fns <- grep('shRNA', fns, invert = TRUE, value = TRUE)
    fns <- grep('Liver', fns, invert = TRUE, value = TRUE)
...
## names(tmp) <- gsub('.bed.gz', '', names(tmp))
## names(tmp) <- gsub('.bedGraph.gz', '', names(tmp))

names(tmp) <- c('CENP\nNon-activated', 'CENP\nActivated', 'input\nNon-activated', 'input\nActivated',
                 'CpGi', 'H3K27ac', 'ATAC-seq', 'PolII', 'H3K4me3', 'H4K27me3')
...
tryCatch({
    ...
    multiHeatMatrix(tmp[c(1,2,5,6,7)],
                    xcoords = c(-2000, 1000),
                    clustfun = function(x) kmeans(x, centers = 5)$cluster,
                    clust.matrix = 1:2,
                    col = pal)
...
```
    ...
    ## wigs <- grep('input', wigs, invert = TRUE, value = TRUE)
