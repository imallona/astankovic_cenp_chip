...
```{r, cache = FALSE}
saveRDS(test, file = 'test_another.rds')
...
selected <- c('CENP', 'K4me3', 'K27ac', 'Pol2', 'CBP', 'K27me3')
selected %in% test$category
test <- test[test$category %in% selected,]

test$category <- factor(test$category, levels = selected)
```
...
set.seed(134)
test[sample(1:nrow(test), size = 2e5),] %>% 
    ggplot(aes(x = pos, y = value,  color = cluster, group.by = sample, linetype = status)) +
