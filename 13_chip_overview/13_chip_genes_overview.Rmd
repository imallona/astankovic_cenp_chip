...
awk '{OFS=FS="\t"; print $1,$2,$3,$9,$10}' bed/edgeR_dge_results_treatmenttreated_1h-treatmentcontrol_edge.bed > bed/edge.bed


~/soft/bedtools/bedtools-2.29.2/bin/bedtools intersect -a bed/genes.bed  \
                                             -b bed/edge.bed  "$COVS"/chip_HC_1_cov_bga.bedgraph.gz "$COVS"/chip_PTZ_1_cov_bga.bedgraph.gz \
                                             -wao -names edge chip_hc chip_ptz > bed/tmp.bed




# # so they both have 14 fields, edgeR and chip outputs
# sed -i  's/\tchip/\tchip\tchip/g' bed/tmp.bed

~/soft/bedtools/bedtools-2.29.2/bin/bedtools groupby -g 1,2,3,8 -c9 -o distinct | head  

```

```{r notfinishedneither, eval = FALSE}


edge_all <- list()
for (fn in edge_fns) {

    tmp <- read.table(fn, header = TRUE)

    tmp$ensembl <- sapply(strsplit(tmp$gene_id, split = '.', fixed = T), function(x) return(x[1]))
    edge_all[[fn]] <- tmp
    rm(tmp)
}

names(edge_all) <-  gsub('.txt', '',
                         gsub('treatment', '', gsub('edgeR_dge_results_', '', basename(names(edge_all)))))

## mind the coverage filter
## covs_filt <- lapply(covs, function(x) return(x[!is.na(x$cov) & x$cov > 9,]))

export_bed(gr = covs$cenp_HC_1, outfile = 'bed/cenp_hc.bed')
export_bed(gr = covs$cenp_PTZ_1, outfile = 'bed/cenp_ptz.bed')

for (contrast in c("treated_1h-control", "treated_6h-treated_1h", "treated_48h-treated_6h")) {

    ## curr_up <- paste0(contrast, '_up')
    ## curr_down <- paste0(contrast, '_down')
    ## comb <- findFeatureComb(GRangesList(c(covs, edge_pos[curr_up],
    ##                                       edge_neg[curr_down])), width = 500, use.names = TRUE)

    ## comb <- findFeatureComb(GRangesList(c(covs, list(genes = genes_asis))), width = 50, use.names = TRUE)

    export_bed(gr = edge_all[[contrast]], outfile = sprintf('bed/%s_edge.bed', contrast))
    ## export_bed(gr = comb, outfile = sprintf('bed/%s_%s.bed', contrast, 'comb'))

    cmd <- sprintf('~/soft/bedtools/bedtools-2.29.2/bin/bedtools  intersect -a bed/genes.bed  -b bed/%s_comb.bed  -wb -wa > bed/%s_genes.bed', contrast, contrast)
    system(cmd)

    ## overlaps <- import(sprintf('bed/%s_genes.bed', contrast), format="bed")
    overlaps <- read.table(sprintf('bed/%s_genes.bed', contrast), header = FALSE)

    fd <- merge(overlaps, edge_all[[contrast]], by.x = 'V8', by.y = 'ensembl')
    ## expression count table

    get_longest_string <- function(x) {
        return(head(x[rev(order(nchar(x)))], 1))
    }
    
    ## harmonize fd so each gene is only once
    ## shorter <- data.frame(ensembl = unique(fd$ensembl),
    ##                       class = get_
                          
    
    boxplot(logFC ~ V15, data = fd)
    boxplot(logCPM ~ V15, data = fd)
    aggregate(logFC ~ V15, summary, data = fd)
}



    
    ## ## comb <- comb[grepl('genes', comb$class),]
    ## ## grab the whole gene, for those overlaps

    ## ## query, subject
    ## ## ranges <- subsetByOverlaps(query = genes_asis, subject = comb)

    ## ## query, subject
    ## hits <- findOverlaps(query = genes_asis, subject = comb)


    
    ## overlapclass <- CharacterList(split(comb$class[subjectHits(hits)],
    ##                                     queryHits(hits)))
    ...
    ## binding <- sapply(overlapclass, function(x) return(get_longest_string(x)))



    ## mcols(genes_asis) <- DataFrame(mcols(genes_asis),  binding)
    

    
    
##     sml = ScoreMatrixList(c(covs, edge_pos[[curr_up]], edge_neg[[curr_down]]), genes_asis, bin.num = 50,
##                           is.noCovNA = TRUE, strand.aware = TRUE)
##     names(sml) <- c(names(covs), curr_up, curr_down)

##     sml <- intersectScoreMatrixList(sml, reorder = TRUE)

##     sml.scaled = scaleScoreMatrixList(sml)

##     found_row_idx <- dimnames(sml[[1]]@.Data)[[1]]
##     found_gene_id <- genes_asis$gene_id[as.numeric(as.character(found_row_idx))]

##     unique(genes_asis[found_gene_id,]$binding)
##     multiHeatMatrix(sml.scaled, xcoords = c(-250, 250), col = c("lightgray", "blue"),
##                     group = as.factor(genes_asis[found_gene_id,]$binding))
...
## sml = ScoreMatrixList(c(covs, edge_pos, edge_neg), comb, bin.num = 50)

## names(sml) <- c(names(edge_pos), names(edge_neg), names(covs))
