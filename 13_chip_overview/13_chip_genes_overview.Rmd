...
multiHeatMatrix(scaleScoreMatrixList(sml_genes),
                xcoords = c(0, 100), clustfun = function(x) kmeans(x, centers = 3)$cluster,
                order = TRUE,
                clust.matrix = 1:4)
```
...
multiHeatMatrix(sml.scaled,
                xcoords = c(minx, maxx), clustfun = function(x) kmeans(x, centers = 3)$cluster,
                order = TRUE,
                clust.matrix = 1:4)
```
...
## sml_genes_with_cpg <- ScoreMatrixList(c(sml.scaled, sm_cpgi),
...
Not done here, this should be done for promoters - only
...
```{r}
print('From now on, we check genes!')
