...
## Gene expression {.tabset .tabset-pills}
...
set.seed(1)
multiHeatMatrix(sml_genes_with_cpg_edge,
                xcoords = c(minx, maxx),
                group = as.factor(clu[as.character(sml_genes_with_cpg_edge_idx)]),
                group.col = c('forestgreen', 'blue', 'red', 'black', 'orange'),
                ## user.order = 1:5,
                order = TRUE,
                winsorize = c(1,99),
...
set.seed(1)
multiHeatMatrix(harmonize_sml(sml_genes_with_cpg_edge, clu[as.character(sml_genes_with_cpg_edge_idx)]),
                xcoords = c(minx, maxx),
                group = as.factor(clu[as.character(sml_genes_with_cpg_edge_idx)]),
                group.col = c('forestgreen', 'blue', 'red', 'black', 'orange'),
                ## user.order = 1:5,
...
set.seed(1)
multiHeatMatrix(harmonize_sml(sml_genes_with_cpg, clu[as.character(sml_genes_with_cpg_edge_idx)]),
                xcoords = c(minx, maxx),
                clustfun = clu[as.character(sml_genes_with_cpg_edge_idx)],
                group.col = c('forestgreen', 'blue', 'red', 'black', 'orange'),
                ## user.order = 1:5,
                order = FALSE,
                winsorize = c(1,99),
...
multiHeatMatrix(harmonize_sml(sml_genes, clu[as.character(sml_genes_with_cpg_edge_idx)]),
                xcoords = c(minx, maxx),
                group = as.factor(clu[as.character(sml_genes_with_cpg_edge_idx)]),
                group.col = c('forestgreen', 'blue', 'red', 'black', 'orange'),
                ## user.order = 1:5,
...
set.seed(1)
test <- lapply(sml_genes_with_cpg_edge, function(x) return(val2unit(x@.Data)))

for (item in names(test)) {
    fd <- as.data.frame(test[[item]])
    fd$window <- 1:nrow(fd)
    fd$cluster <- paste0('cluster ', clu[as.character(sml_genes_with_cpg_edge_idx)])
    fd$sample <- item
    ## colnames(fd) <- paste0('pos', colnames(fd))
    head(fd)
    fd <- as.data.frame(fd) %>% 
...
```{r, fig.width = 12, fig.height = 4}
set.seed(1)
multiHeatMatrix(c(sml_genes_scaled_with_cpg_edge, bin.num = 100, windows = genes_asis),
                xcoords = c(minx, maxx),
                clustfun = clu_scaled[as.character(sml_genes_scaled_with_cpg_edge_idx)],
                group.col = c('forestgreen', 'blue', 'red', 'black', 'orange'),
                user.order = 1:5,
                order = TRUE,
                winsorize = c(1,99),
...
```{r, fig.width = 10, fig.height = 6}
set.seed(1)
test <- lapply(sml_genes_scaled_with_cpg_edge, function(x) return(val2unit(x@.Data)))

for (item in names(test)) {
    fd <- as.data.frame(test[[item]])
    fd$window <- 1:nrow(fd)
    fd$cluster <- paste0('cluster ', clu_scaled[sml_genes_scaled_with_cpg_edge_idx])
    fd$sample <- item
    ## colnames(fd) <- paste0('pos', colnames(fd))
    head(fd)
    fd <- as.data.frame(fd) %>% 
