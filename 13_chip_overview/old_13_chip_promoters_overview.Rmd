...
## covs_fns <- grep('1_cov', covs_fns, value = TRUE)
...
Remove replicates 23

```{r}
covs <- covs[c('cenp_HC_1', 'cenp_PTZ_1')]
```
...
sm_genes_cpgi <- ScoreMatrixBin(target = do.call(c, cpgi_mm10),
                          windows = genes_asis, strand.aware = TRUE,
                          bin.num = 50)


print("check if we're double scaling!")
## sml_with_cpg <- ScoreMatrixList(c(sml.scaled, sm_cpgi),
##                                 genes_asis, bin.num = 50,
##                                 strand.aware = TRUE,
##                                 cores = NTHREADS)

sml_genes_with_cpg <- ScoreMatrixList(c(scaleScoreMatrixList(sml_genes), sm_genes_cpgi),
                                genes_asis, bin.num = 50,
                                strand.aware = TRUE,
                                cores = NTHREADS)
...
plotMeta(sml_genes, profile.names = names(sml_genes),
         ...
         smoothfun = NULL,
                  centralTrend = 'mean',
         dispersion = 'se', xcoords = c(0,1), xlab = 'normalized position (0 means TSS, 1 TES)')
```
...
plotMeta(sml_genes, profile.names = names(sml_genes),
         meta.rescale = TRUE,
         lwd = 2,
         smoothfun = NULL, xcoords = c(0,1),  xlab = 'normalized position (0 means TSS, 1 TES)')
```
...
## Diff gene expression upon activation, Fernandez-Albert

Are genes diff regulated upon activation have to do with CENP clusters? Not much, by a preliminary inspection?
...
names(edge_pos) <-  paste0('up_', gsub('.txt_up', '',
                         gsub('treatment', '', gsub('edgeR_dge_results_', '', basename(names(edge_pos))))))
names(edge_neg) <-  paste0('down_', gsub('.txt_down', '',
...
Plotted are absolute logFCs for each contrast, stratified by being up/down
...
What if clustering by gene expression, i.e. very early genes up or down in 1h KCl vs ctrl?
...
<!-- plotMeta(c(sml, edge), profile.names = c(names(sml), names(edge))) -->
...
for (fn in fns) {
    bws[[basename(fn)]] <- tryCatch({
        tmp <- readBigWig(fn, windows = genes_proms,
                          type = "auto",  strand.aware = TRUE)

        seqlevelsStyle(tmp) <- "UCSC"
        
        ScoreMatrixBin(tmp, windows = genes_proms,
                    type = "auto",
                    bin.num = 50,
                    strand.aware = TRUE)
    }, error = function(x) {
        ...
        seqlevelsStyle(tmp) <- "UCSC"
        return(ScoreMatrixBin(tmp, genes_proms_ncbi,
                                              bin.num = 50,
                                              type = "auto",  strand.aware = TRUE))
...
```{r meanswider, eval = TRUE, message = FALSE, cache = FALSE}

## mat.list = lapply(foo, function(x) x@.Data)

mat.list = lapply(intersectScoreMatrixList(c(sml_with_cpg, GSE125068, peaks, lung_liver_cenp, GSE21161), reorder = TRUE),
                  function(x) x@.Data)
...
for (samp in names(mat.list)) {
    ...
    mat.list[[samp]][ is.na(mat.list[[samp]]) ] <- 0
...
<!-- longer$category[grepl('K27ac', longer$sample)] <- 'K27ac' -->
...
First, quality control, which are the clusters we're talking about (as stratified metaplots)?


```{r, fig.width = 22, fig.height = 6, cache = FALSE}
multiHeatMatrix(c(sml_with_cpg, GSE125068, peaks, lung_liver_cenp, GSE21161),
...
mat.list = lapply(intersectScoreMatrixList(c(sml, GSE125068, peaks, lung_liver_cenp, GSE21161), reorder = TRUE),
                  function(x) x@.Data)

names(mat.list) <- names(c(sml, GSE125068, peaks, lung_liver_cenp, GSE21161))
longer <- list()
for (samp in names(mat.list)) {
    ## table(tidyr::pivot_longer(data = as.data.frame(mat.list[[samp]]), cols = 1:ncol(mat.list[[samp]]))$name)
    longer[[samp]] <- list()
...
longer$category[grepl('K27ac', longer$sample)] <- 'eg_K27ac'
longer$category[grepl('CBP', longer$sample)] <- 'eg_CBP'
...
longer$panel[grepl('K4me3', longer$category)] <- 'K4me3'
longer$panel[grepl('K4me1', longer$category)] <- 'active'
longer$panel[grepl('Pol2', longer$category)] <- 'RNA'
longer$panel[grepl('ATAC', longer$category)] <- 'active'
longer$panel[grepl('K27ac', longer$category)] <- 'active'
...
longer$panel[grepl('SRF', longer$category)] <- 'SRF/CREB/CBP'
longer$panel[grepl('CREB', longer$category)] <- 'SRF/CREB/CBP'
longer$panel[grepl('CBP', longer$category)] <- 'SRF/CREB/CBP'

longer$category <- gsub('eg_', '', gsub('ex_', '', longer$category))



longer$activation <- 'control'
longer$activation[grepl('KCl', longer$sample)] <- 'KCl'
longer$activation[grepl('KA1h', longer$sample)] <- 'KA1h'
longer$activation[grepl('KA6h', longer$sample)] <- 'KA6h'
longer$activation[grepl('KA1h', longer$sample)] <- 'KA1h'
longer$activation[grepl('KA48h', longer$sample)] <- 'KA48h'
longer$activation[grepl('PTZ', longer$sample)] <- 'PTZ'
longer$activation[grepl('NE', longer$sample)] <- 'NE'


## table(longer$category, longer$panel, longer$activation, useNA = 'ifany')

```

<!-- ```{r, fig.width = 8, fig.height = 10} -->
<!-- ggplot(data = longer, aes(x = pos, y = value, color = activation)) + -->
<!--     geom_smooth(method = "loess") + -->
<!--     facet_grid(category~cluster) + -->
<!--     theme_bw() + -->
...
ggplot(data = longer, aes(x = pos, y = value, color = category, group.by = sample, linetype = activation)) +
...
```{r, cache = FALSE}
## mat.list = lapply(c(sml, GSE125068, peaks, lung_liver_cenp, GSE21161), function(x) x@.Data)
mat.list = lapply(intersectScoreMatrixList(c(sml, GSE125068, peaks, lung_liver_cenp, GSE21161), reorder = TRUE),
                  function(x) x@.Data)
...
## ggplot(data = longer, aes(x = pos, y = value, color = category, group.by = sample, linetype = activation)) +
##     geom_smooth(method = "loess", se = FALSE) +
##     facet_grid(panel~cluster, scales = "free_y") +
##     theme_bw() +
##     scale_x_discrete('distance to TSS (bp)',
##                      breaks = c(0, 25, 50), labels = c('-2000', '0', '-2000'), limits = c(0, 25, 50)) +
##     scale_color_brewer(palette="Paired") +
