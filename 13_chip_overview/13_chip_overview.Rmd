...
multiHeatMatrix(sml.scaled,
                xcoords = c(minx, maxx), clustfun = function(x) kmeans(x, centers = 5)$cluster,
                order = TRUE,
                clust.matrix = 1:4)
```
...
Get the coverages where there is the top signal, i.e. quantile 90 (top 10%) etc?
...
This is wrong because it's the wrong assembly!!
...
What about meta.rescaling genomation-style? This looks best
...
```{r}
mat.list = lapply(c(sml_with_cpg, GSE125068, peaks, lung_liver_cenp), function(x) x@.Data)

names(mat.list) <- names(c(sml_with_cpg, GSE125068, peaks, lung_liver_cenp))
longer <- list()
for (samp in names(mat.list)) {
    ## table(tidyr::pivot_longer(data = as.data.frame(mat.list[[samp]]), cols = 1:ncol(mat.list[[samp]]))$name)
    longer[[samp]] <- list()
