...
<!-- Genes shorter than 200 bp removed - so we don't get things with CENP/promoter signal all across the genebody. -->

<!-- ```{r} -->
<!-- genes_asis <- genes_asis[width(genes_asis) >=200,] -->
<!-- ``` -->
...
## clustering based on HC_1 and PTZ_1
clust.matrix = 1:2
...
```{r fig.height = 4, fig.width = 8, results = 'asis', cache = FALSE}

enrich <- list()

for (quant in c(0.2, 0.5, 0.8, 0.9, 0.925, 0.95, 0.975, 0.99)) {

    cat('## Quantile', quant, ' {.tabset .tabset-pills} \n\n')
    sel <- list()
    for (item in names(covs)) {
    ...
    print(genomicElementDistribution(GRangesList(sel), 
                                    TxDb = txdb,
                                    ignore.strand = FALSE,
                               ...
                               ##                 CDS = "#FFFFFF",
                               ##                 otherExon = "#FFFFFF"),
                               promoterRegion=c(upstream=1000, downstream=1000),
                               geneDownstream=c(upstream=0, downstream=2000)))

    
    peaks <- genomicElementDistribution(GRangesList(sel), 
                                      TxDb = txdb,
                                      ignore.strand = FALSE,
                                      labels = list(geneLevel = c(promoter = "Promoter",
                                                                  geneDownstream = "Downstream",
    ...
    foo <- list(cenp_hc_1 = as.data.frame(mcols(peaks$peaks$cenp_HC_1)),
                ## cenp_hc_23 = as.data.frame(mcols(peaks$peaks$cenp_HC_23)),
                cenp_ptz_1 = as.data.frame(mcols(peaks$peaks$cenp_PTZ_1)))#,
                ## cenp_ptz_23 = as.data.frame(mcols(peaks$peaks$cenp_PTZ_23)))

    foo$cenp_hc_1$origin <- 'cenp_hc_1'
    foo$cenp_ptz_1$origin <- 'cenp_ptz_1'
   
    ## foo$cenp_hc_23$origin <- 'cenp_hc_23'
    ## foo$cenp_ptz_23$origin <- 'cenp_ptz_23'
    
    foo <- do.call(rbind.data.frame, foo)
    foo$quantile <- quant
    

    ## ggplot(foo, aes(fill=geneLevel, y=cov, x=origin)) + 
    ##     geom_bar(position="fill", stat="identity") +
    ##     theme_bw()

    foo$geneLevel <- gsub('distalIntergenic', 'intergenic', foo$geneLevel)

    
    enrich[[as.character(quant)]] <- foo %>% group_by(geneLevel, origin) %>% 
        summarize(count = length(geneLevel))

    cat('\n\n')
    
    cat('### custom \n\n')
    
    print(foo %>% group_by(geneLevel, origin) %>% 
        summarize(count = length(geneLevel)) %>%
        ggplot(aes(fill=geneLevel, y=count, x=origin)) + 
        geom_bar(position="fill", stat="identity") +
        theme_bw() +
        ylab('proportion') +
        xlab('condition') +
        coord_flip() +
        ggtitle(sprintf('Quantile %s', quant)))
...
Caption: Genomic distribution of CENP-A-bound sites stratified by signal intensity; percentile 99 depicts the CENP-A-bound regions with the 1% highest signal (read counts).

Caption2: Genomic distribution of CENP-A-bound sites stratified by signal intensity; percentile 99 depicts the top 1% signal (i.e. ChIP read counts).



```{r, fig.width = 7, fig.height = 4}

## do.call(rbind.data.frame, enrich)
for (quant in names(enrich)) {
    enrich[[quant]] <- as.data.frame(enrich[[quant]])
    enrich[[quant]]$percentile <- as.numeric(quant)*100
}

enrich$percentile <- as.character(enrich$percentile)
enrich <- do.call(rbind.data.frame, enrich)

saveRDS(object = enrich, file = 'enrich.rds')

p <- ggplot(enrich, aes(x=percentile, y = count, fill = geneLevel)) +
    geom_bar(position="fill", stat="identity") +
    facet_wrap(~origin) +
    ylab('proportion of CENP-A signal') +
    xlab('signal strength percentile') +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

print(p)

ggsave(p, filename = 'enrichment.svg', width = 6, height = 4)
ggsave(p, filename = 'enrichment.png', width = 6, height = 4)
...
## edge_sm <- ScoreMatrixList(edge, genes_asis, bin.num = 100, weight.col = 'logFC')
...
# On just summing reads in promoters, for some promoter definition {.tabset .tabset-pills}
...
## doesn't matter which contrast, chip is chip/regardless of edgeR
wide$delta_cenp_reads_23 <- wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_PTZ_2_3.txt.cenp_nreads` -
    wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_HC_2_3.txt.cenp_nreads`

wide$delta_cenp_reads_1 <- wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_PTZ_1.txt.cenp_nreads` -
    wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_HC_1.txt.cenp_nreads`

```

Normalize by 1 M centromeric reads


```{r}
## ~/cenp_chip/nested_mapping/chip_HC_1_oligomerized/log
hc_1_centrom <- 0.5978 * 49536148
## ~/cenp_chip/nested_mapping/chip_PTZ_1_oligomerized/log
ptz_1_centrom <- 0.6888*38482901

counts_per_million_centromeric_reads <- function(x, centrom_total) {
    return(x * centrom_total / 1e6)
}

counts_per_million_centromeric_reads(1, centrom_total = hc_1_centrom)
counts_per_million_centromeric_reads(1, centrom_total = ptz_1_centrom)

```{r}
wide$delta_cenp_cpmc_1 <- counts_per_million_centromeric_reads(wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_PTZ_1.txt.cenp_nreads`, ptz_1_centrom) -
    counts_per_million_centromeric_reads(wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_HC_1.txt.cenp_nreads`, hc_1_centrom)



## mind the sample swap for reps 2 and 3 before adding them

```


Note: `delta_cenp_reads_1` means the direct `cenp_chip_PTZ_1 - cenp_chip_HC_1` subtract value; whereas `delta_cenp_cpmc_1` depicts the same delta PTZ vs HC counts per promoter, but transformed/normalized as `counts per million centromeric reads`. Of course, PTZ_1 and HC_1 have (each) their own number of mapped centromeric reads as scaling factor.

Data for replicate 23 are not shown in this report.

```{r, results = 'asis', fig.width = 5, fig.height = 5}

for (chip in c('chip_HC_1', 'chip_PTZ_1')) {
    
    cat('## ', chip, ' {.tabset .tabset-pills}\n\n')
    
    for (item_short in c('treatmenttreated_1h-treatmentcontrol',
                   'treatmenttreated_6h-treatmenttreated_1h',
                   'treatmenttreated_48h-treatmenttreated_6h')) {
        cat('### ', item_short, ' {.tabset .tabset-pills} \n\n')
        
        item <- paste0(item_short, '__', chip)
       

        for (viewpoint in c('logCPM', 'logFC')) {
            cat('#### ', viewpoint, ' {.tabset .tabset-pills}  \n\n')
            c(grep(item, colnames(wide), value =  TRUE))
              
            curr <- wide[,c(grep(item, colnames(wide), value =  TRUE))]

            colnames(curr) <- gsub(sprintf('edgeR_dge_results_%s.txt.', item), '',
                                   colnames(curr))

            k <- ceiling(max(abs(curr[, viewpoint]), na.rm = TRUE))

            if (viewpoint == 'logCPM')
                curr$bin <- cut(curr[, viewpoint], breaks = seq(-k, +k, by = 1))
            if (viewpoint == 'logFC')
                curr$bin <- cut(curr[, viewpoint], breaks = seq(-k, +k, by = 2))
            ## curr$bin <- cut(curr[, viewpoint], breaks = 10)

            cat('\n\n##### median\n\n')
            print(ggplot(curr, aes(x = bin, y =  cenp_medianreads)) +
                  geom_point(alpha = 0.1, size = 1) +
                  ## geom_point() +
                  geom_violin() +
                  theme_bw() +
                  scale_y_continuous(trans='log1p', breaks = c(0, 1, 5, 10, 100, 500, 2000, 5000, 10000)) +
                  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
                  xlab(sprintf('expression %s in %s', viewpoint, item_short)) +
                  stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")+
                  stat_summary(fun= function(x) median(x) - mad(x),
                               geom="point", shape=18, size=3, color="darkblue", fill="darkblue") +
                  stat_summary(fun= function(x) median(x) + mad(x),
                               geom="point", shape=18, size=3, color="darkblue", fill="darkblue")
            ...
            cat('\n\n##### sum\n\n')
            print(ggplot(curr, aes(x = bin, y =  cenp_nreads)) +
                  geom_point(alpha = 0.1, size = 1) +
                  ## geom_point() +
                  geom_violin() +
                  theme_bw() +
            ...
            cat('\n\n##### mean\n\n')
            print(ggplot(curr, aes(x = bin, y =  cenp_meanreads)) +
                  geom_point(alpha = 0.1, size = 1) +
                  ## geom_point() +
                  geom_violin() +
                  theme_bw() +
...
wide$`treatmenttreated_1h-treatmenttreated_control.logFC` <- wide$`edgeR_dge_results_treatmenttreated_1h-treatmentcontrol__chip_HC_1.txt.logFC`

wide$`treatmenttreated_1h-treatmenttreated_control.logCPM` <- wide$`edgeR_dge_results_treatmenttreated_1h-treatmentcontrol__chip_HC_1.txt.logCPM`

wide$`treatmenttreated_6h-treatmenttreated_1h.logFC` <- wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_PTZ_2_3.txt.logFC`
wide$`treatmenttreated_6h-treatmenttreated_1h.logCPM` <- wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_PTZ_2_3.txt.logCPM`

wide$`treatmenttreated_48h-treatmenttreated_6h.logFC` <- wide$`edgeR_dge_results_treatmenttreated_48h-treatmenttreated_6h__chip_PTZ_2_3.txt.logFC`
wide$`treatmenttreated_48h-treatmenttreated_6h.logCPM` <- wide$`edgeR_dge_results_treatmenttreated_48h-treatmenttreated_6h__chip_PTZ_2_3.txt.logCPM`
...
## https://stackoverflow.com/questions/20924705/plot-negative-values-in-logarithmic-scale-with-ggplot-2
...
for (delta in c("delta_cenp_reads_1", "delta_cenp_cpmc_1")) {
    
    cat('## ', delta, ' {.tabset .tabset-pills}\n\n')
    
    for (item_short in c('treatmenttreated_1h-treatmenttreated_control',
                   'treatmenttreated_6h-treatmenttreated_1h',
                   'treatmenttreated_48h-treatmenttreated_6h')) {
        cat('### ', item_short, ' {.tabset .tabset-pills} \n\n')
        ...
        for (viewpoint in c('logCPM', 'logFC')) {
            item <- paste0(item_short, '.', viewpoint)
            
            cat('#### ', viewpoint, ' {.tabset .tabset-pills}  \n\n')
            ## c(grep(item, colnames(wide), value =  TRUE))
              
            curr <- wide[,c(grep(item, colnames(wide), value =  TRUE), delta)]


            k <- ceiling(max(abs(curr[, item]), na.rm = TRUE))

            if (viewpoint == 'logCPM')
                curr$bin <- cut(curr[, item], breaks = seq(-k, +k, by = 1))
            if (viewpoint == 'logFC')
                curr$bin <- cut(curr[, item], breaks = seq(-k, +k, by = 2))
            ## curr$bin <- cut(curr[, viewpoint], breaks = 10)

            ## cat('\n\n##### median\n\n')
            ## library(ggallin)
            print(ggplot(curr, aes(x = bin, y =  get(delta))) +
                  geom_point(alpha = 0.1, size = 1) +
                  ## geom_point() +
                  ...
                  theme_bw() +
                  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
                  xlab(sprintf('%s %s', viewpoint, item_short)) +
                  ylab(delta) +
                  stat_summary(fun=mean, geom="point", shape=18, size=3, color="red", fill="red")+
                  stat_summary(fun= function(x) median(x) - mad(x),
                               geom="point", shape=18, size=3, color="darkblue", fill="darkblue") +
...
# Counts per million centromeric reads PTZ_1 vs HC_1

In promoters, here


```{r}
tmp <- data.frame(cpcm = c(wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_PTZ_1.txt.cenp_nreads`/ ptz_1_centrom, wide$`edgeR_dge_results_treatmenttreated_6h-treatmenttreated_1h__chip_HC_1.txt.cenp_nreads`/ hc_1_centrom),
                  status = c(rep('PTZ', nrow(wide)), rep('HC', nrow(wide))))
print(ggplot(tmp, aes(x=status, y=cpcm, fill = status)) +
      geom_violin() +
      scale_y_log10() +
      theme_bw())
...
The one with expression, CBP, K27ac, Pol2 etc (but not K4me3)
...
This is the one with H3K4me3 and H3K4me1

The public data were peaks, not signal/bws. mm10, ncbi-style chrom names. We've re-aligned them.

<!-- ```{r old_peaks, eval = FALSE} -->
<!-- gse <- 'GSE85873' -->


<!-- fns <- file.path(PUBLIC, gse, list.files(file.path(PUBLIC, gse), pattern = '.*gz$')) -->

<!-- ## fns <- grep('H3K4me', fns, value = TRUE) -->
...
```{r bws_k4me3, eval = TRUE}
gse <- 'GSE85873'

reproc <- '/home/imallona/cenp_chip/public/k4me3/bamCoverage'
fns <- file.path(reproc, list.files(file.path(reproc), pattern = '.*bw$'))

fns <- grep('filtered', fns, value = TRUE, invert = TRUE)
...
names(bws) <- gsub('novel_environment', 'ne', names(bws))
names(bws) <- gsub('k4me3', 'K4me3', names(bws))
...
bws <- ScoreMatrixList(bws, bin.num = 100)
names(bws) <- gsub('.seq_depth_norm.bw', '', names(bws))
...
can we resort clusters?

```{r, fig.height = 6, fig.width = 25}

sorting <- as.factor(clu[tmp_idx])

foo <- sort(factor(sorting, levels = 1:NUM_CLUSTERS))

multiHeatMatrix(harmonize_sml(tmp, foo),
                xcoords = c(minx, maxx),
                group = foo,
                order = TRUE,
                winsorize = c(1,99),
...
```{r, cache = FALSE}
saveRDS(test, file = 'test_genes_another_rep1.rds')

## test <- readRDS('test_another.rds')
## unique(test$sample)
...
```{r}
test$category <- 'else'
test$category[grepl('ChIP_H3K27ac_Sal', test$sample)] <- 'eg_K27ac'
...
test$category[grepl('K27me3', test$sample)] <- 'eg_K27me3'


test$category[grepl('Pol2', test$sample)] <- 'ex_Pol2'
test$category[grepl('PolII', test$sample)] <- 'ex_Pol2'
...
test$category[grepl('CBP', test$sample)] <- 'eg_CBP'
...
test$panel[grepl('K27ac', test$category)] <- 'active'
test$panel[grepl('K27me3', test$category)] <- 'inactive'
...
test$panel[grepl('CBP', test$category)] <- 'CBP'
...
## head(test[test$category == 'else',])
```

```{r}

## selected <- c('CENP', 'ATAC', 'K4me3', 'K27ac', 'Pol2', 'CBP', 'K27me3')
## selected_metaplot <- c('CENP', 'K27ac', 'K4me3', 'Pol2', 'CBP', 'nuRNA')
## selected_heatmap <- c('CENP', 'K27ac', 'K27me3')

selected_metaplot <- c('CENP',   'K4me3', 'Pol2', 'CBP', 'nuRNA', 'ATAC')
selected_heatmap <- c('CENP', 'K27ac', 'K27me3')


selected <- selected_metaplot

## selected_heatmap_samples <-  names(tmp) %in% unique(test[test$category %in% selected_heatmap, 'sample'])

## tmp_selected <- tmp[selected_heatmap_samples]

selected_heatmap_samples <- as.data.frame(unique(test[test$category %in% selected_heatmap, 'sample']))$sample
                                        
tmp_selected <- tmp[names(tmp) %in% selected_heatmap_samples]

test <- test[test$category %in% selected_metaplot,]

test$category <- factor(test$category, levels = selected_metaplot)
```

```{r}
test <- test[!test$sample %in% 'GSM2442440_5789_Neuron_PolII_Rep1.bedGraph.gz',]

## for metaplotting the 'PTZ' is removed
## test <- test[!test$sample %in% 'cenp_PTZ_1',]
```


```{r}
test$value[is.na(test$value)] <- 0
```

<!-- ```{r, fig.width = 10, fig.height = 10} -->

<!-- set.seed(134) -->
<!-- test[sample(1:nrow(test), size = 2e5),] %>%  -->
<!--     ggplot(aes(x = pos, y = value, color = cluster, group.by = sample)) + -->
<!--     geom_smooth(method = "loess", se = FALSE) + -->
<!--     facet_grid(category~cluster, scales = "free") + -->
<!--     theme_bw() + -->
<!--     scale_x_discrete('normalized genebody position', -->
...
names(tmp_selected) <- gsub('GSM2442457_N-K27me3-1.bedGraph.gz', 'ChIP_K27me3',
                            names(tmp_selected))
...
multiHeatMatrix(harmonize_sml(tmp_selected, foo),
                xcoords = c(minx, maxx),
                group = foo,
                order = TRUE,
                winsorize = c(1,99),
...
Requested order/requested panels
...
test$status[grepl('ne', test$sample)] <- 'activated'
test$status[grepl('ctrl', test$sample)] <- 'control'
...
test$status[test$sample %in% c('GSM2442440_5789_Neuron_PolII_Rep1.bedGraph.gz',
                               'GSM2442457_N-K27me3-1.bedGraph.gz')] <- 'control'

test$status[grepl('wt_NE', test$sample)] <- 'activated'

unique(test[test$status == 'else', 'sample'])
table(test$sample, test$status)
```

Sampling, loess smoothing

```{r, fig.width = 10, fig.height =7}
set.seed(134)
test[sample(1:nrow(test), size = 2e5),] %>% 
    ggplot(aes(x = pos, y = value,  color = status, group.by = sample)) +
    geom_smooth(method = "loess", se = FALSE, span = 0.15) +
    facet_grid(category~cluster, scales = "free") +
    theme_bw() +
    scale_x_discrete('normalized genebody position',
                     breaks = c(10, 90), labels = c('TSS', 'TES'), limits = c(0, 10, 90, 100)) +
    scale_color_brewer(palette="Set1") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
test %>%
    group_by(cluster, sample, pos, status, category) %>%
    summarise_at(vars(value),
               list(value = median)) %>% 
    ggplot(aes(x = pos, y = value,  color = status, group.by = sample)) +
    geom_line(lwd = 1) +
    facet_grid(factor(category, levels = selected)~cluster, scales = "free") +
    theme_bw() +
    scale_x_discrete('normalized genebody position',
                     breaks = c(10, 90), labels = c('TSS', 'TES'), limits = c(0, 10, 90, 100)) +
    scale_color_brewer(palette="Set1") +
...
```{r}
winsorize<-function(mat, rang){
  hi.th <- quantile(mat, rang[2]/100,na.rm = TRUE)
  lo.th <- quantile(mat, rang[1]/100,na.rm = TRUE)
  mat[mat>hi.th] = hi.th
  mat[mat<lo.th] = lo.th
  return(mat)
}
...
test %>%
    group_by(cluster, sample, pos, status, category) %>%
    summarise_at(vars(value),
               list(value = function(x) median(winsorize(x, rang = c(1, 99))))) %>% 
    ggplot(aes(x = pos, y = value,  color = status, group.by = sample)) +
    geom_line(lwd = 1) +
    facet_grid(factor(category, levels = selected)~cluster, scales = "free") +
    theme_bw() +
...
test %>%
    group_by(cluster, sample, pos, status, category) %>%
    summarise_at(vars(value),
               list(value = function(x) mean(winsorize(x, rang = c(1, 99))))) %>% 
    ggplot(aes(x = pos, y = value,  color = status, group.by = sample)) +
    geom_line(lwd = 1) +
    facet_grid(factor(category, levels = selected)~cluster, scales = "free") +
    theme_bw() +
...
<!--     group_by(cluster, sample, pos, status, category) %>% -->
<!--     summarise_at(vars(value), -->
<!--                list(value = function(x) median(winsorize(x, rang = c(2, 8))))) %>%  -->
<!--     ggplot(aes(x = pos, y = value,  color = status, group.by = sample)) + -->
<!--     geom_line(lwd = 1) + -->
<!--     facet_grid(factor(category, levels = selected)~cluster, scales = "free") + -->
<!--     theme_bw() + -->
...
<!--     geom_smooth(method = "loess", se = TRUE, span = 0.05) +  -->
<!--     facet_grid(factor(category, levels = selected)~cluster, scales = "free") + -->
<!--     theme_bw() + -->
<!--     scale_x_discrete('normalized genebody position', -->
<!--                      breaks = c(10, 90), labels = c('TSS', 'TES'), limits = c(0, 10, 90, 100)) + -->
...
<!-- covs <- ScoreMatrixList(covs, bin.num = 100, cores = NTHREADS) -->
...
<!-- <\!-- edge <- ScoreMatrixList(edge_sm, bin.num = 100, cores = NTHREADS) -\-> -->
<!-- <\!-- ``` -\-> -->
...
<!-- <\!-- ```{r} -\-> -->
<!-- <\!-- ## ?ScoreMatrix -\-> -->
<!-- <\!-- sml <- ScoreMatrixList(covs, wtranscripts, bin.num = 100, type = "auto",  strand.aware = TRUE, cores = NTHREADS, -\-> -->
<!-- <\!--                        is.noCovNA = TRUE) -\-> -->
<!-- <\!-- ``` -\-> -->
...
<!-- <\!-- curr <- ScoreMatrixList(curr, bin.num = 100) -\-> -->
