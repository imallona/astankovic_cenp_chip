...
```{r noscaling, fig.width = 8, fig.height = 6, dev = 'png'}
set.seed(1)
...
## edge_sm <- ScoreMatrixList(edge, genes_proms, bin.num = 100, weight.col = 'logFC')
...
test %>% group_by(sample, cluster, pos) %>% 
    ...
    facet_wrap(~cluster) +
    theme_bw() +
     scale_x_continuous(breaks = c(0,50,100), labels = c('-1kb', 'TSS', '+1kb')) + theme(aspect.ratio=1)
...
export_bed(genes_proms, outfile = 'bed/genes.bed')
```
...
These are peaks, not signal. mm10, ncbi-style chrom names.  Updated: mapped the H4me3, using our bigWigs instead
...
peaks <- bws ## old comp
tmp <- intersectScoreMatrixList(ScoreMatrixList(c(sml_genes_with_cpg, GSE125068, ScoreMatrixList(peaks)),
                                                genes_proms,
                                                bin.num = 100,
                                                strand.aware = TRUE,
                                                cores = NTHREADS),
...
```{r, fig.height = 6, fig.width = 25, dev = 'png'}
...
```{r, cache = FALSE}
saveRDS(test, file = 'test_promoters_another_rep1.rds')
...
## selected_metaplot <- c('CENP', 'ATAC', 'K4me3', 'Pol2', 'CBP')
## selected_heatmap <- c('CENP', 'CpGi', 'CENP_liver', 'ATAC', 'K4me3', 'K27ac', 'Pol2', 'CBP',  'K27me3')

selected_metaplot <- c('CENP', 'K4me3', 'Pol2',  'CBP', 'ATAC')
selected_heatmap <- c('CENP', 'CENP_liver',  'CpGi', 'K27ac', 'K27me3')

selected <- selected_metaplot
## selected_heatmap_samples <-  names(tmp)[names(tmp) %in%
##                                         unique(test[test$category %in%
##                                                                  selected_heatmap, 'sample'])
...
test[sample(1:nrow(test), size = 2e5),] %>% 
    ...
    facet_grid(category~cluster, scales = "free") +
    theme_bw() +
    scale_x_discrete('promoter position',
                     breaks = c(0, 50, 100), labels = c('-1kb', 'TSS', '+1 kb'), limits = c(0, 50, 100)) +
    scale_color_brewer(palette="Set1") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
<!-- <\!-- ## joint <- ScoreMatrixList(list(sml_genes_with_cpg, GSE125068, GSE85873, GSE21161), targets = genes_proms, -\-> -->
<!-- <\!-- ##                          bin.num = 100) -\-> -->
